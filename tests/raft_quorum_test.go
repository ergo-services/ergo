...
type testQuorumRaft struct {
	gen.Raft
	res chan interface{}
}

func (tr *testQuorumRaft) InitRaft(process *gen.RaftProcess, args ...etf.Term) (gen.RaftOptions, error) {
...
func (tr *testQuorumRaft) HandleQuorum(process *gen.RaftProcess, q *gen.RaftQuorum) gen.RaftStatus {
	if q == nil {
...
func (tr *testQuorumRaft) HandleLeader(process *gen.RaftProcess, leader *gen.RaftLeader) gen.RaftStatus {
	fmt.Println("LLL leader", process.Name(), leader)
	return gen.RaftStatusOK
}

func (tr *testQuorumRaft) HandleAppend(process *gen.RaftProcess, ref etf.Ref, serial uint64, key string, value etf.Term) gen.RaftStatus {
	fmt.Println("AAA append", ref, serial, value)
	return gen.RaftStatusOK
}

func (tr *testQuorumRaft) HandleGet(process *gen.RaftProcess, serial uint64) (string, etf.Term, gen.RaftStatus) {
	fmt.Println("GGG get", process.Name(), serial)
	return "", nil, gen.RaftStatusOK
}

func (tr *testQuorumRaft) HandleRaftInfo(process *gen.RaftProcess, message etf.Term) gen.ServerStatus {
	q := process.Quorum()
