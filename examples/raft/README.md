...
1. Starting 4 nodes with RRR processes. They are connecting to each other to build a quorum (invoking HandleQuorum callback on each raft process). Since we use 4 raft processes, only 3 of them will be chosen for the quorum (gen.RaftQuorumState3). One of them becomes a follower (it won't receive the leader election result).
2. Quorum is built. They start leader election among the quorum members.
...
4. Raft process checks the missing serial on its storage and makes a Get request to the cluster to receive them.
...
7. Raft process checks the missing serial on its storage and makes a Get request to the cluster to receive them (follower had no chance to make this check since it doesn't receive information with the election leader result)
