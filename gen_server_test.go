...
// This test is checking cases below:
//
...
func TestGenServer(t *testing.T) {
	...
	fmt.Printf("    process.Send local (gs1) -> local (gs2) : ")
	node1gs1.Send(node1gs2.Self(), etf.Atom("hi"))
	waitForResult(t, gs2.err)

	node1gs1.Cast(node1gs2.Self(), etf.Atom("hi cast"))
	fmt.Printf("    process.Cast local (gs1) -> local (gs2) : ")
	waitForResult(t, gs2.err)

	fmt.Printf("    process.Call local (gs1) -> local (gs2): ")
	v := etf.Atom("hi call")
	...
	fmt.Printf("    process.Send local (gs1) -> remote (gs3) : ")
	node1gs1.Send(node2gs3.Self(), etf.Atom("hi"))
	waitForResult(t, gs3.err)

	node1gs1.Cast(node2gs3.Self(), etf.Atom("hi cast"))
	fmt.Printf("    process.Cast local (gs1) -> remote (gs3) : ")
	waitForResult(t, gs3.err)

	fmt.Printf("    process.Call local (gs1) -> remote (gs3): ")
