...
* Unmarshalling terms into the struct using `etf.TermIntoStruct`, `etf.TermMapIntoStruct`, `etf.TermProplistIntoStruct` or to the string using `etf.TermToString`
...
* Added support of Erlang/OTP 24 (with [Alias](https://blog.erlang.org/My-OTP-24-Highlights/#eep-53-process-aliases) support)
...
* Introduced new methods for `Node`:
  * `ProvideSpawnRemote`, `RevokeSpawnRemote`, `SpawnRemote`, `CancelSpawnRemote` spawn process on a remote node (within Erlang/OTP 24 support)
