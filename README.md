...
* Introduced new interfaces
  ...
  - `Handshake`
  - `Proto`/`Connection`
* Improved sending messages: methods `gen.Process.Send`, `gen.ServerProcess.Cast`, `gen.ServerProcess.Call` now return `node.ErrProcessIncarnation` if message is addressed to the process of previous incarnation (remote node has been restarted).
