...
func TestGenStageSimple(t *testing.T) {
	...
	producerProcess, errP := node.Spawn("stageProducer", ProcessOptions{}, producer, nil)
	if errP != nil {
		t.Fatal(errP)
	}
	consumerProcess, errC := node.Spawn("stageConsumer", ProcessOptions{}, consumer, nil)
	if errC != nil {
		t.Fatal(errC)
	}
	fmt.Println("OK")
	...
	subOpts := GenStageSubscribeOptions{
		...
		Cancel: GenStageCancelTemporary,
	}
	...
	fmt.Println("OK")
	fmt.Printf("... Consumer sent 'Ask' request with count = 1 (min 2, max 4). GenStage shouldn't call HandleDemand : ")
	consumer.Ask(consumerProcess, sub4, 1)
	...
	waitForTimeout(t, consumer.value) // shouldn't receive anything here
	fmt.Println("OK")
	fmt.Printf("... Consumer sent yet another 'Ask'. GenStage shouldn't call HandleDemand as well: ")
...
func TestGenStageDistributed(t *testing.T) {
	...
	fmt.Printf("... starting Producer and Consumer processes: ")
	producerProcess, errP := node1.Spawn("stageProducer", ProcessOptions{}, producer, nil)
	if errP != nil {
		t.Fatal(errP)
	}
	consumerProcess, errC := node2.Spawn("stageConsumer", ProcessOptions{}, consumer, nil)
	if errC != nil {
		t.Fatal(errC)
	}
	fmt.Println("OK")
	...
	sub := consumer.Subscribe(consumerProcess, etf.Tuple{"stageProducer", "nodeGenStageDistributed01@localhost"}, subOpts)
	...
	// case 2: GenStageCancelTransient

	fmt.Printf("... Starting Producer process (test GenStageCancelTransient) : ")
	producerProcess1, errP1 := node1.Spawn("stageProducer", ProcessOptions{}, producer, nil)
	if errP1 != nil {
		t.Fatal(errP1)
	}
	fmt.Println("OK")

	subOpts.Cancel = GenStageCancelTransient
	sub1 := consumer.Subscribe(consumerProcess, etf.Tuple{"stageProducer", "nodeGenStageDistributed01@localhost"}, subOpts)
	fmt.Printf("... Producer@node1 handled subscription request from Consumer@node2: ")
	waitForResultWithValue(t, producer.value, sub1)
	fmt.Printf("... Consumer@node2 handled subscription confirmation from Producer@node1 (GenStageCancelTransient): ")
	...
	// case 3: GenStageCancelPermanent

	fmt.Printf("... Starting Producer process (test GenStageCancelPermanent) : ")
	producerProcess2, errP2 := node1.Spawn("stageProducer", ProcessOptions{}, producer, nil)
	if errP2 != nil {
		t.Fatal(errP2)
	}
	consumerProcess1, errC := node2.Spawn("stageConsumer", ProcessOptions{}, consumer, nil)
	if errC != nil {
		t.Fatal(errC)
	}
	fmt.Println("OK")

	subOpts.Cancel = GenStageCancelPermanent
	sub2 := consumer.Subscribe(consumerProcess1, etf.Tuple{"stageProducer", "nodeGenStageDistributed01@localhost"}, subOpts)
	fmt.Printf("... Producer@node1 handled subscription request from Consumer@node2: ")
	waitForResultWithValue(t, producer.value, sub2)
	fmt.Printf("... Consumer@node2 handled subscription confirmation from Producer@node1 (GenStageCancelPermanent): ")
	waitForResultWithValue(t, consumer.value, sub2)

	producerProcess2.Exit(producerProcess2.Self(), "normal")
	...
	_, errP3 := node1.Spawn("stageProducer", ProcessOptions{}, producer, nil)
	if errP3 != nil {
		t.Fatal(errP3)
	}
	consumerProcess2, errC := node2.Spawn("stageConsumer", ProcessOptions{}, consumer, nil)
	if errC != nil {
		t.Fatal(errC)
	}
	fmt.Println("OK")
	subOpts.Cancel = GenStageCancelTemporary
	sub3 := consumer.Subscribe(consumerProcess2, etf.Tuple{"stageProducer", "nodeGenStageDistributed01@localhost"}, subOpts)
	fmt.Printf("... Producer@node1 handled subscription request from Consumer@node2: ")
	waitForResultWithValue(t, producer.value, sub3)
	fmt.Printf("... Consumer@node2 handled subscription confirmation from Producer@node1 (GenStageCancelTemporary): ")
...
func TestGenStageDispatcherDemand(t *testing.T) {
	...
	producer := &GenStageProducerTest{
		value: make(chan interface{}, 2),
		// GenStageDispatcherDemand - this is default dispatcher, so we shouldn't create it explicitly
		// dispatcher: CreateGenStageDispatcherDemand(),
	}
	consumer := &GenStageConsumerTest{
		value: make(chan interface{}, 2),
	}
	fmt.Printf("... starting Producer (GenStageDispatcherDemand): ")
	producerProcess, errP := node.Spawn("stageProducer", ProcessOptions{}, producer, nil)
	if errP != nil {
		t.Fatal(errP)
	}
	...
	consumer1Process, errC1 := node.Spawn("stageConsumer1", ProcessOptions{}, consumer, nil)
	if errC1 != nil {
		t.Fatal(errC1)
	}
	fmt.Println("OK")
	...
	consumer2Process, errC2 := node.Spawn("stageConsumer2", ProcessOptions{}, consumer, nil)
	if errC2 != nil {
		t.Fatal(errC2)
	}
	fmt.Println("OK")
	...
	consumer3Process, errC3 := node.Spawn("stageConsumer3", ProcessOptions{}, consumer, nil)
	if errC3 != nil {
		t.Fatal(errC3)
	}
	fmt.Println("OK")
...
func TestGenStageDispatcherBroadcast(t *testing.T) {
	...
	producer := &GenStageProducerTest{
		value:      make(chan interface{}, 2),
		dispatcher: CreateGenStageDispatcherBroadcast(),
	}
	consumer1 := &GenStageConsumerTest{
		value: make(chan interface{}, 2),
	}
	consumer2 := &GenStageConsumerTest{
		value: make(chan interface{}, 2),
	}
	consumer3 := &GenStageConsumerTest{
		value: make(chan interface{}, 2),
	}
	fmt.Printf("... starting Producer (GenStageDispatcherBroadcast): ")
	producerProcess, errP := node.Spawn("stageProducer", ProcessOptions{}, producer, nil)
	if errP != nil {
		t.Fatal(errP)
	}
	fmt.Println("OK")
	fmt.Printf("... starting Consumer1: ")
	consumer1Process, errC1 := node.Spawn("stageConsumer1", ProcessOptions{}, consumer1, nil)
	if errC1 != nil {
		t.Fatal(errC1)
	}
	fmt.Println("OK")
	...
	fmt.Printf("... starting Consumer2: ")
	consumer2Process, errC2 := node.Spawn("stageConsumer2", ProcessOptions{}, consumer2, nil)
	if errC2 != nil {
		t.Fatal(errC2)
	}
	fmt.Println("OK")
	...
	fmt.Printf("... starting Consumer3: ")
	consumer3Process, errC3 := node.Spawn("stageConsumer3", ProcessOptions{}, consumer3, nil)
	if errC3 != nil {
		t.Fatal(errC3)
	}
	fmt.Println("OK")
...
func TestGenStageDispatcherPartition(t *testing.T) {
	...
	subOpts1 := GenStageSubscribeOptions{
		MinDemand: 3,
		MaxDemand: 4,
		Partition: 0,
	}
	subOpts2 := GenStageSubscribeOptions{
		MinDemand: 3,
		MaxDemand: 4,
		Partition: 1,
	}
	subOpts3 := GenStageSubscribeOptions{
		MinDemand: 3,
		MaxDemand: 4,
		Partition: 2,
	}
	...
	producer := &GenStageProducerTest{
		value:      make(chan interface{}, 2),
		dispatcher: CreateGenStageDispatcherPartition(3, hash),
	}
	consumer1 := &GenStageConsumerTest{
		value: make(chan interface{}, 2),
	}
	...
	fmt.Printf("... starting Producer (GenStageDispatcherPartition): ")
	producerProcess, errP := node.Spawn("stageProducer", ProcessOptions{}, producer, nil)
	if errP != nil {
		t.Fatal(errP)
	}
